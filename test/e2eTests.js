...
describe("TellorFlex e2e Tests", function () {
    ...
    it("TBR, stakes, and staking rewards reach 0", async function () {
        // Setup
        let bigStake = web3.utils.toWei("1000000")
        await token.mint(accounts[1].address, bigStake + BigInt(web3.utils.toWei("2")))
        await token.connect(accounts[1]).approve(tellor.address, bigStake + BigInt(web3.utils.toWei("2")))

        // Check balance after adding time-based rewards, stake, & staking rewards
        await token.mint(tellor.address, web3.utils.toWei("100")) // add tb rewards
        await tellor.connect(accounts[1]).depositStake(bigStake)
        await tellor.connect(accounts[1]).addStakingRewards(web3.utils.toWei("1"))
        tellorBalance1 = await token.balanceOf(tellor.address)
        expect(tellorBalance1).to.equal(BigInt(bigStake) + BigInt(web3.utils.toWei("101")))
        ...
        await tellor.connect(accounts[1]).submitValue(TRB_QUERY_ID, h.uintTob32(420), 0, TRB_QUERY_DATA)
        rewardsGiven = web3.utils.toWei("100") - await tellor.totalTimeBasedRewardsBalance()
        expect(await token.balanceOf(tellor.address)).to.equal(BigInt(bigStake) + BigInt(web3.utils.toWei("1")))

        // Check balance after zeroing stakes & staking rewards
        await tellor.connect(accounts[1]).requestStakingWithdraw(bigStake)
        h.advanceTime(60 * 60 * 24 * 7) // 7 days
        await tellor.connect(accounts[1]).withdrawStake()
        h.advanceTime(60 * 60 * 24 * 7) // 7 days
        // await tellor.connect(accounts[1]).requestStakingWithdraw(web3.utils.toWei("1"))
        // expect(await token.balanceOf(tellor.address)).to.equal(web3.utils.toWei("1"))
    })
})